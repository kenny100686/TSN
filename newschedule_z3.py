...
#讀取所有TT flow資訊並紀錄
fp = open('flow/TT1/Limited_flow_data-%d.txt'%(ttfilename), 'r')
for i in fp:
    globals()["tt{}".format(n+1)] = []  # from tt1 to ttn+1 = []
...
# 求hyper period of all tt flows
#pair = combine(tt_count, 2)
#print(pair)
for i in range(len(tmp_list)): # tmp_list save tt flows' period
...
for i in fo:
    ...
    b = i.rstrip('\n') # ExtoEy
    #print("b is ", b)
    globals()["{}".format('l'+str(i.rstrip('\n')))] = [0]*hyper_period   # lExtoEy = [0, 1, 0], link的time slot, hyper_period as upper bound
    ...
    globals()["{}_slot".format(str(i.strip('\n')))] = [0] # ExtoEy_slot = [0], 儲存此link用了多少的slot數量
    a = 'l'+str(i.rstrip('\n'))
...
for i in range(len(tt_count)):
    ...
    tti = eval(tti_name)
    #print(tti)
    count_in_hyperperiod = int(hyper_period/int(tti[0])) # repeated times of tti in one hyper_period
    ...
    for k in range(len(path)-1):  #將tt經過的每條link區隔開來
        link_pass.append(path[k:k+2])   #path[k, k+1] appends to link_pass
        link_name = link_pass[k][0]+'to'+link_pass[k][1]    #path[k, k+1] as link, i.e. ExtoEy

        #print('link_name is',link_name) 
        tmp_cal = link_dict[link_name]   #取出目前link的權重數 (the times which link be used?)
        tmp_cal = tmp_cal+1 
        link_dict[link_name] = tmp_cal
        link_dict.update()

        #記錄每條link是哪些tt經過
        link_record = "tt"+link_name    #ttExtoEy
        #print("link_record", link_record)
        link_record = eval(link_record)
        link_record.append(str(i+1))
        print('link_record is ', link_record)

print('link_dict is ', link_dict)
sorted_link_weight = {}
sorted_link_weight = sorted(link_dict.items(), key = itemgetter(1), reverse = True) # 對dict中第1維的元素進行降序排序 
...
print('not_sorted_link is', not_sorted_link)
...
while not_sorted_link:    #如果還有link沒有進行排程,則不能結束
    ...
    print("schedule_link = ", schedule_link)   #ttExEy = [1, 2, 3 ...], 每條link是哪些tt經過
    ...
    if len(schedule_link)>0:   #若link上有尚未進行排成的tt則進行最佳化
        ...
        opt = Optimize()    # model
        print('num of constraint while starting = ', len(opt.assertions()))
        ...
        pair = []
        pair = combine(tmp_schedule_tt, 2)  #因為每個tt要互相排程,所以將所有組合產出 (must?)
        #print("pair", pair)
        #宣告每個TT的offset變數
        ...
        for numbers in range(count_schedule_tt):
            ...
            tti = eval(a)
            va = "x"+str(x_number+1)    #x1, x2, x3 ....
            
            #globals()['x{}'.format(x_number+1)] = m.addVar(lb = 0, ub = int(int(tti[0])-1), vtype = GRB.INTEGER, name = va)  #offset變數宣告完畢
            globals()['x{}'.format(x_number+1)] = Int(va)  #offset變數宣告完畢
            opt.add(eval(va) >= 0, eval(va) <= int(int(tti[0])-1))  #offset constraint
            tti.append(eval(va))
            x_number = x_number+1
            ...
            lname = 'l'+path[0][0]+'to'+path[1][0]  # lExtoEy = [0, 1, 0, ...], link的time slot, hyper_period as upper bound
            lname = eval(lname)
            print('lname: ', lname)
            '''
            lE3toE7[0] = 1
            ...
            if position:
                for notuse in range(len(position)):
                    ...
                    globals()['n{}'.format(n_number)] = Int(na)  #na變數宣告完畢
                    opt.add(eval(na) >= 0, eval(na) <= int(hyper_period))  #na constraint                    
                    na = eval(na)                    
                    print('na is', na)
                    ...
                    globals()['y{}'.format(y_number)] = Int(ya)  #ya變數宣告完畢                    
                    opt.add(eval(ya) >= 0, eval(ya) <= 1)  #ya constraint
                    ya = eval(ya)                    
                    y_number = y_number+1
                    ...
                    opt.add(na == int(tmp_number))  
                    n_number = n_number+1
                    c_number = c_number+1
                    
                    # offset <= na - e or offset >= na + e, where na is unavailable time slot
                    # =====|-----|-----|=====>    
                    #     na-e   na   na+e

                    ca = "c"+str(c_number)
                    #m.addConstr(na-tti[6]+M*ya>=e, ca)  #tti[6] = tti.offset
                    opt.add(na-tti[6]+M*ya>=e)  
                    c_number = c_number+1
                    ca = "c"+str(c_number)
                    #m.addConstr(na-tti[6]+M*ya<=M-e, ca)
                    opt.add(na-tti[6]+M*ya<=M-e) 
                    c_number = c_number+1
        ...
        ttinumber = x_number  # 紀錄產生了多少個offset變數,注意x_number,假設link上有4條tt要排成,則現在的x_number為4
        for numbers in range(count_schedule_tt):
            a = "tt"+str(tmp_schedule_tt[numbers])
            tti = eval(a)
            print(a)
            tti_L = (tti[3]+29)*8/1000  #頻寬的值這邊統一為1Gbps,算出來的單位為us, tti[3] = tti.length
            tti.append(tti_L)
            print(tti)
            ca = "c"+str(c_number)
            c_number = c_number +1
            #m.addConstr(tti[6]<=tti[0]-tti[7]-0.096, ca)    # tti[6] = tti.offset, tti[0] = tti.period, tti[7] = tti.L 
            opt.add(tti[6]<=tti[0]-tti[7]-0.096) 
            #print('1')
        ...
        for i in pair:
            ...
            tmp_varaible = "tt"+not_sorted_link[0]  # ttExtoEy
            ...
            if tti_n_hop > 0:
                for ttith in range(tti_n_hop):
                    ...
                    tti_A = tti_A + tti[7] + topology_3[tmp_src][tmp_dest]['propDelay']  # tti[7] = tti.L 
            else:
                pass
            ...
            for k in range(a):
                for l in range(b):
                    ...
                    globals()['x{}'.format(x_number+1)] = Int(va)  #變數宣告完畢
                    opt.add(eval(va) >= 0, eval(va) <= 1)  
                    #宣告c0 c1 c2等編號
                    ca = "c"+str(c_number)
                    c_number = c_number+1
                    ...
                    opt.add(ttj[6]-tti[6]+l*ttj[0]-k*tti[0]-M*eval(va)<= tti_A-(ttj_B+ttj[7]+0.096)) 
                    ...
                    opt.add(tti[6]-ttj[6]+k*tti[0]-l*ttj[0]+M*eval(va)<= M+ttj_B-tti_A-(tti[7]+0.096)) 
                    x_number = x_number+1
        ...
        for i in range(count_schedule_tt):
            ...
            tti = eval(tt_i)
            a = int(hyper_period/int(tti[0]))
            obj = obj+tti[6]*a  # tti[6] = tti.offset

        '''
        m.setObjective(obj, GRB.MINIMIZE)   # obj is min(summation of all tti.offset)
        m.update()
        m.optimize()
        '''
        opt_value = opt.minimize(obj)
        ...
        if(opt.check()):
            z3_model = opt.model()
            print(z3_model)
            print ('opt value = ', opt_value.value())
            print('opt function = ', opt.objectives())
        #print(opt.check())
        #print(opt.model())
        ...
        # Assign new offset which calculated using Gurobi
        '''
        offsetname = []
        for i in range(count_schedule_tt):
            s = 'x'+str(i+1)
            offsetname.append(s)
        print('var_list = ', m.getVars())
        for v in m.getVars():
            for n in range(len(offsetname)):
                if offsetname[n] == v.varName:
                    ...
                    tti = eval(tti)
                    tti[6] = int(v.x)   # tti[6] = tti.offset  
                    print(tti)
                else:
                    pass
        '''
        ...
        for i in range(count_schedule_tt):
            s = 'x'+str(i+1)
            offsetname.append(s)
        #print('var_list = ', model.getVars())
        print('offsetname = ', offsetname)
        for v in z3_model:
            for n in range(len(offsetname)):
        ...
        print('num of constraint while ending = ', len(opt.assertions()))
        ...
        for i in range(count_schedule_tt):
            ...
            for timess in range(int(int(hyper_period)/operating_tt[0])):
                tmp_array.append([operating_tt_name, operating_tt[6]+timess*operating_tt[0]])   # operating_tt[6] = tti.offset
        print('tmp_array = ', tmp_array)    #tmp_array = [['tt1', 1], ['tt1', 101], ['tt4', 0], ['tt4', 100], ['tt5', 4], ['tt6', 3]]
        ...
        for i in range(length_link_group_tt):
            ...
            operating_tt = eval(operating_ttname)   #tti
            print("目前要排成 ", operating_tt)
            tt_start = 'E'+ str(operating_tt[1]) #tti[1] = src
            tt_end = 'E' + str(operating_tt[2]) #tti[2] = dest
            
            #宣告tti_QD變數
            tti_qd = operating_ttname+"_QD" #tti_QD
            ttiqd = eval(tti_qd)
            ...
            for p in range(len(path)):
                ...
                path.pop(0)
            print("處理過的path", path)
            first_offset = operating_tt[6]  #tti[6] = tti.offset
            #找出要推算的link
            hop = len(path)
            for nodeth in range(hop-1):
                ...
                if path[nodeth] in hostnode:
                    ...
                    linkname = "l"+nodesrc+"to"+nodedest    #lExtoEy
                    ...
                    ttoffset = link_group_tt[i][1]  #link_group_tt is [['tt1',0],['tt2',2],['tt1',100],['tt2',102]]
                    endtrans = ttoffset+operating_tt[7]+0.096 #該tt從offset開始到傳送結束的時間, tti[7] = tti.L
                    ttoffsetkeep = math.ceil(endtrans)-ttoffset #math.ceil()回傳大於等於所給數字的最小整數
                    ...
                    #佔用的time slot記錄下來,其它條TT不能佔用, linkname = lExtoEy = [0, 1, 0], link的time slot, hyper_period as upper bound
                    for used in range(ttoffsetkeep):

                        linkname[ttoffset+used] = linkname[ttoffset+used] + 1

                    nexthop_tt_start_time = ttoffset + operating_tt[7] + linkpropagationdelay   #tti[7] = tti.L
                    print(linkname)
                    ...
                    for listvariable in xmlentry:
                        ...
                        else:   #overlap, 將發送時間向後移動, but we have already calculated the schedule, why shifting? 
                            print("shift")
                    ...
                    if s2 == tmpuse:    #no overlap, tmpuse = ttoffset + operating_tt[7] + linkpropagationdelay
                        ...
                        if (e2floor-s2floor)>0: #排程的傳輸時間超出一格
                            if transfloor > (1 - interframegap): #自身傳輸時間就需要超出一格,不動作
                                ...
                                gate_open_time = math.ceil(s2)  #floor?
...
#列印出每條link上tt flows' offset值的狀態
linkname_start_time = []    #record tti start time on a specific link
linkname_end_time = []  #record tti end time on a specific link
wrong_schedule = 0  #flag
fq = open('topology_information.txt', 'r')
for ll in fq:
    wrong_schedule = 0
    linkname = "nodeto"+ll.rstrip('\n')
    ...
    #verify the schedule, link by link
    if(linkname):
        linkname_start_time.clear()
        linkname_end_time.clear()

        #record all tt flows' start time and end time on a specific link 
        for tti in range(len(linkname)): 
            linkname_start_time.append(linkname[tti]['start'])
            linkname_end_time.append(linkname[tti]['end'])
        linkname_start_time.sort()
        print("linkname_start_time is ", linkname_start_time)
        linkname_end_time.sort()
        print("linkname_end_time is ", linkname_end_time)

        #check violation, i.e. e1 > s2
        for i in range(len(linkname_end_time)-1):
            if(linkname_end_time[i] > linkname_start_time[i+1]):
                wrong_schedule = 1

    if(wrong_schedule == 1):
        print('\033[91m'+'wrong schedule'+'\033[0m')
        break
    else:
        print('\033[92m'+'correct schedule'+'\033[0m')
